...
class UniRepLKNet(nn.Module):
    ...
    Args:
        ...
        init_cfg (str): optional path to pretrained weights. Default: None
        ...
        **kwargs,
        ...
        if self.is_classification:
            ...
            if init_cfg is not None:
                ckpt = torch.load(init_cfg, map_location="cpu")
    ...
    def reset_classifier(self, num_classes: int, global_pool: Optional[str] = None):
        ...
        self.is_classification = num_classes is not None and num_classes > 0
        self.output_mode = "logits" if self.is_classification else "features"
        self.head = (
    ...
    def forward_intermediates(
        ...
        if torch.jit.is_scripting() or not stop_early:
            stage_range = range(len(self.stages))
        else:
            stage_range = range(max_index + 1)
        for feat_idx in stage_range:
            x = self.downsample_layers[feat_idx](x)
            x = self.stages[feat_idx](x)
    ...
    def prune_intermediate_layers(
        ...
        self.downsample_layers = self.downsample_layers[: max_index + 1]
        if prune_head:
...
def _create_unireplknet(variant, pretrained=False, **kwargs):
...
def _cfg(url="", hf_hub_id="", hf_hub_filename="", **kwargs):
    return {
        "url": url,
        "hf_hub_id": hf_hub_id,
        "hf_hub_filename": hf_hub_filename,
        "num_classes": 1000,
        ...
        "first_conv": "downsample_layers.0.0",
        "classifier": "head",
        "license": "apache-2.0",
        **kwargs,
    }
...
def _cfg_22k(url="", hf_hub_id="", hf_hub_filename="", **kwargs):
    return _cfg(url, hf_hub_id, hf_hub_filename, num_classes=21841, **kwargs)


def _cfg_384(url="", hf_hub_id="", hf_hub_filename="", **kwargs):
    return _cfg(
        url,
        hf_hub_id,
        hf_hub_filename,
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "unireplknet_l.in22k_ft_in1k": _cfg_384(
            ...
            hf_hub_id="DingXiaoH/UniRepLKNet",
            hf_hub_filename="unireplknet_l_in22k_to_in1k_384_acc87.88.pth",
        ),
        "unireplknet_xl.in22k": _cfg_22k(
            ...
            hf_hub_id="DingXiaoH/UniRepLKNet",
            hf_hub_filename="unireplknet_xl_in22k_pretrain.pth",
        ),
        "unireplknet_xl.in22k_ft_in1k": _cfg_384(
            ...
            hf_hub_id="DingXiaoH/UniRepLKNet",
            hf_hub_filename="unireplknet_xl_in22k_to_in1k_384_acc87.96.pth",
        ),
    }
)
